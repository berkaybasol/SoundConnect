...
public final class MarketplaceRequests {
    ...
    public record Update(
            ...
            @CodePointLength(max=120) String title, @CodePointLength(max=4000) String description,
            UUID categoryId, @CodePointLength(max=80) String brand, @CodePointLength(max=100) String model,
    ...
    public record Filter(@CodePointLength(max=100) String q, UUID categoryId, UUID cityId, UUID districtId,
                         Condition condition, @PositiveOrZero Long minPriceMinor,
                         @PositiveOrZero Long maxPriceMinor, SortOrder sort) {}
    public record Report(@NotNull ReportReason reason, @CodePointLength(max=1000) String description,
    ...
    public record Review(@NotNull @PositiveOrZero @JsonDeserialize(using=MarketplaceIntegerDeserializer.class) Long expectedVersion,
                         ...
                         @NotBlank @CodePointLength(min=5,max=1000) String resolutionNote) {}
}
