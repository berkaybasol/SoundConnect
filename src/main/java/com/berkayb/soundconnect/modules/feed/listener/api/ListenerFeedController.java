...
/** Listener audience boundary over the shared feed, delivery and moderation engine. */
@RestController
@RequiredArgsConstructor
@ConditionalOnProperty(prefix = "app.feed.musician", name = "enabled", havingValue = "true", matchIfMissing = true)
@RequestMapping("/api/v1/feed/listener")
@PreAuthorize("hasRole('LISTENER')")
public class ListenerFeedController {
    ...
    public ResponseEntity<BaseResponse<MusicianFeedPageResponse>> get(
        ...
        return privatePage(feed.getForListener(viewerId, limit, cursor, supportedItemTypes),
                "Dinleyici akışı getirildi.");
    }
    ...
    public ResponseEntity<BaseResponse<MusicianFeedMutedAuthorsResponse>> mutedAuthors(
            ...
            @RequestParam(required = false) Integer limit,
            @RequestParam(required = false) String cursor) {
        return privatePage(mutedAuthors.getForListener(userId(principal), limit, cursor),
    ...
    public ResponseEntity<BaseResponse<MusicianFeedFeedbackResponse>> feedback(
        ...
        return ok(feedback.recordItemForListener(viewerId, itemId, request), "Akış tercihi kaydedildi.");
    }
    ...
    public ResponseEntity<BaseResponse<MusicianFeedFeedbackResponse>> mute(
        ...
        UUID viewerId = userId(principal);
        rateLimitGuard.checkFeedback(viewerId);
        return ok(feedback.muteForListener(viewerId, profileType, profileId), "Profil akışta sessize alındı.");
    }
    ...
    public ResponseEntity<BaseResponse<Void>> unmute(
        ...
        UUID viewerId = userId(principal);
        rateLimitGuard.checkFeedback(viewerId);
        feedback.unmuteForListener(viewerId, profileType, profileId);
    ...
    public ResponseEntity<BaseResponse<MusicianFeedTelemetryResponse>> recordEvent(
        ...
        return ok(telemetry.recordForListener(viewerId, request), "Akış olayı kaydedildi.");
    }
