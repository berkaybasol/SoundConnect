...
/** Venue audience boundary over the shared feed, delivery and moderation engine. */
@RestController
@RequiredArgsConstructor
@ConditionalOnProperty(prefix = "app.feed.musician", name = "enabled", havingValue = "true", matchIfMissing = true)
@RequestMapping("/api/v1/feed/venue")
@PreAuthorize("hasRole('VENUE')")
public class VenueFeedController {
    ...
    public ResponseEntity<BaseResponse<MusicianFeedPageResponse>> get(
        ...
        return privatePage(feed.getForVenue(viewerId, limit, cursor, supportedItemTypes),
                "Mekân akışı getirildi.");
    }
    ...
    public ResponseEntity<BaseResponse<MusicianFeedMutedAuthorsResponse>> mutedAuthors(
            ...
            @RequestParam(required = false) Integer limit,
            @RequestParam(required = false) String cursor) {
        return privatePage(mutedAuthors.getForVenue(userId(principal), limit, cursor),
    ...
    public ResponseEntity<BaseResponse<MusicianFeedFeedbackResponse>> feedback(
        ...
        return ok(feedback.recordItemForVenue(viewerId, itemId, request), "Akış tercihi kaydedildi.");
    }
    ...
    public ResponseEntity<BaseResponse<MusicianFeedFeedbackResponse>> mute(
        ...
        UUID viewerId = userId(principal);
        rateLimitGuard.checkFeedback(viewerId);
        return ok(feedback.muteForVenue(viewerId, profileType, profileId), "Profil akışta sessize alındı.");
    }
    ...
    public ResponseEntity<BaseResponse<Void>> unmute(
        ...
        UUID viewerId = userId(principal);
        rateLimitGuard.checkFeedback(viewerId);
        feedback.unmuteForVenue(viewerId, profileType, profileId);
    ...
    public ResponseEntity<BaseResponse<MusicianFeedTelemetryResponse>> recordEvent(
        ...
        return ok(telemetry.recordForVenue(viewerId, request), "Akış olayı kaydedildi.");
    }
