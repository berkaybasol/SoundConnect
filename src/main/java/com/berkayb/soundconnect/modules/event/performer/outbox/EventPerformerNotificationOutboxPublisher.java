...
@Slf4j
public class EventPerformerNotificationOutboxPublisher {
		...
		if (TransactionSynchronizationManager.isActualTransactionActive()
			...
			TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
				...
				public void afterCommit() {
					dispatchKeys.forEach(EventPerformerNotificationOutboxPublisher.this::schedule);
				}
			});
		} else {
			...
			log.warn("Event performer notification outbox has no transaction synchronization; scheduler will dispatch");
		}
	}
		...
		try {
			dispatchCoordinator.trySchedule(key.eventId(), () -> {
				try {
					...
					log.warn(
							"Event performer notification immediate dispatch task failed; durable row remains recoverable. eventId={}, type={}, exceptionType={}",
			...
			log.warn(
					"Event performer notification immediate dispatch was rejected; scheduler will retry. eventId={}, type={}, exceptionType={}",
					key.eventId(), key.type(), exception.getClass().getName()
			);
		}
	}
