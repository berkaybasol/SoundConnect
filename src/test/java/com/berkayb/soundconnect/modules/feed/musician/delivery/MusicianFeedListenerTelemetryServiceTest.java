...
class MusicianFeedListenerTelemetryServiceTest {
    ...
    @Test
    void listenerImpressionIsRecordedAgainstItsAuthoritativeDeliveryInTheSharedTelemetryLedger() {
        ...
        var delivery = new MusicianFeedDeliveredItem(deliveryId, viewer, UUID.randomUUID(), "TRACK:fixture",
                ...
                "FOLLOWING_PUBLICATION", Set.of(MusicianFeedFeedbackAction.HIDE), 1, "listener-v1.0.0", 0,
        ...
        var request = request();
        var response = service.recordForListener(viewer, request);
    ...
    void listenerCannotReuseHistoricalBusinessOrMusicianFeedReceiptsForTelemetry() {
        for (String algorithm : List.of("musician-v1.2.0", "venue-v1.0.0")) {
            var delivery = new MusicianFeedDeliveredItem(UUID.randomUUID(), viewer, UUID.randomUUID(), "TRACK:former-role",
                    ...
                    null, "{}", NOW.minusSeconds(1), NOW.plusSeconds(60), NOW.plusSeconds(120));
            when(deliveries.require("signed.delivery", viewer, null, NOW)).thenReturn(delivery);
            assertThatThrownBy(() -> service.recordForListener(viewer, request()))
    ...
    @Test
    void unapprovedListenerIsRejectedBeforeReadingOrWritingTelemetry() {
    ...
    @Test
    void admittedListenerStillNeedsItsOwnValidSignedDeliveryBeforeAnyTelemetryWrite() {
        doThrow(new SoundConnectException(ErrorType.BAD_REQUEST)).when(deliveries)
                .require("signed.delivery", viewer, null, NOW);
        assertThatThrownBy(() -> service.recordForListener(viewer, request())).isInstanceOf(SoundConnectException.class);
        var order = inOrder(viewers, deliveries);
        order.verify(viewers).requireListenerProfile(viewer);
