package com.berkayb.soundconnect.modules.feed.listener.api;
...
@WebMvcTest(controllers = ListenerFeedController.class, excludeFilters = @ComponentScan.Filter(
        type = FilterType.ASSIGNABLE_TYPE,
        classes = com.berkayb.soundconnect.auth.security.JwtAuthenticationFilter.class))
@AutoConfigureMockMvc(addFilters = false)
@Import(ListenerFeedControllerTest.MethodSecurity.class)
class ListenerFeedControllerTest {
    ...
    @Test
    void listenerPageUsesSharedContractAndAudienceSpecificServiceAfterContinuationBudget() throws Exception {
        UUID viewer = authenticate("ROLE_LISTENER");
        UUID session = UUID.randomUUID();
        when(feed.getForListener(viewer, 20, "signed.cursor", List.of("TRACK", "COLLAB")))
                .thenReturn(new MusicianFeedPageResponse(1, "listener-v1.0.0", session,
                        Instant.parse("2026-09-14T12:00:00Z"), List.of(), "next.cursor", true));
        mvc.perform(get("/api/v1/feed/listener").param("limit", "20").param("cursor", "signed.cursor")
        ...
        order.verify(feed).getForListener(viewer, 20, "signed.cursor", List.of("TRACK", "COLLAB"));
    ...
    void pageRequiresRendererCapabilitiesAndPropagatesInvalidAudienceCursorWithoutData() throws Exception {
        UUID viewer = authenticate("ROLE_LISTENER");
        mvc.perform(get("/api/v1/feed/listener")).andExpect(status().isBadRequest());
        verifyNoInteractions(feed, rateLimitGuard);
        when(feed.getForListener(eq(viewer), isNull(), eq("musician.cursor"), eq(List.of("TRACK"))))
                .thenThrow(new SoundConnectException(ErrorType.BAD_REQUEST));
        mvc.perform(get("/api/v1/feed/listener").param("cursor", "musician.cursor").param("supportedItemTypes", "TRACK"))
                .andExpect(status().isBadRequest()).andExpect(jsonPath("$.data").doesNotExist());
    }

    @Test
    void mutedListUsesListenerAuthorizationAndPrivateEnvelope() throws Exception {
        UUID viewer = authenticate("ROLE_LISTENER");
        when(mutedAuthors.getForListener(viewer, 30, "muted.cursor"))
                .thenReturn(new MusicianFeedMutedAuthorsResponse(List.of(), null, false));
        mvc.perform(get("/api/v1/feed/listener/muted-authors").param("limit", "30").param("cursor", "muted.cursor"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.items").isEmpty())
                .andExpect(header().string("Cache-Control", containsString("private")));
        verify(mutedAuthors).getForListener(viewer, 30, "muted.cursor");
        verifyNoMoreInteractions(mutedAuthors);
    }

    @ParameterizedTest
    @EnumSource(value = Route.class, names = {"FEEDBACK", "MUTE", "UNMUTE", "TELEMETRY"})
    void writesUseTheSameAccountBudgetAndListenerGuardedBusinessPath(Route route) throws Exception {
        UUID viewer = authenticate("ROLE_LISTENER");
        ...
        switch (route) {
            case FEEDBACK -> order.verify(feedback).recordItemForListener(viewer, ITEM, FEEDBACK);
            case MUTE -> order.verify(feedback).muteForListener(viewer, "MUSICIAN", TARGET);
            case UNMUTE -> order.verify(feedback).unmuteForListener(viewer, "MUSICIAN", TARGET);
            case TELEMETRY -> order.verify(telemetry).recordForListener(viewer, TELEMETRY);
    ...
    void everyListenerRouteRejectsMusiciansOtherRolesAndAnonymousBeforeServices(Route route) throws Exception {
        for (String role : List.of("ROLE_MUSICIAN", "ROLE_VENUE", "ROLE_STUDIO", "ROLE_ADMIN")) {
            authenticate(role);
    ...
    private MockHttpServletRequestBuilder request(Route route) {
        return switch (route) {
            case PAGE -> get("/api/v1/feed/listener").param("supportedItemTypes", "TRACK");
            case MUTED -> get("/api/v1/feed/listener/muted-authors");
            case FEEDBACK -> post("/api/v1/feed/listener/items/{id}/feedback", ITEM).contentType("application/json")
                    .content("{\"action\":\"HIDE\",\"impressionToken\":\"signed.delivery\"}");
            case MUTE -> put("/api/v1/feed/listener/authors/MUSICIAN/{id}/mute", TARGET);
            case UNMUTE -> delete("/api/v1/feed/listener/authors/MUSICIAN/{id}/mute", TARGET);
            case TELEMETRY -> post("/api/v1/feed/listener/events").contentType("application/json")
    ...
    private UUID authenticate(String role) {
        ...
        var user = User.builder().id(viewer).username("listener_feed").password("unused")
                .email("listener-feed@example.test").emailVerified(true).status(UserStatus.ACTIVE)
