...
class VenueFeedServiceTest {
    ...
    private final UUID viewer = UUID.randomUUID(), venue = UUID.randomUUID(), city = UUID.randomUUID();
    ...
    @BeforeEach void prepare() {
        when(guard.requireVenueProfile(viewer)).thenReturn(venue);
        when(personalization.loadForVenue(viewer, venue)).thenReturn(new MusicianFeedPersonalizationSnapshot(city, Set.of(), null));
        ...
        when(deliveries.recordPage(eq(viewer), any(), any(), eq(1), eq("venue-v1.0.0"), anyLong(), anyList(), anyList(), eq(NOW)))
                .thenAnswer(call -> {
    ...
    @Test void venueUsesSharedPageEngineAndForwardsAudienceWithoutCompletionCapability() {
        ...
        var completion = provider("completion", Set.of(MusicianFeedItemType.PROFILE_COMPLETION), request -> {
            throw new AssertionError("Venue must never request profile completion");
        });
        var page = service(tracks, completion).getForVenue(viewer, 1, null, List.of("TRACK", "PROFILE_COMPLETION"));
        assertThat(page.items()).hasSize(1);
        assertThat(page.algorithmVersion()).isEqualTo("venue-v1.0.0");
        assertThat(page.hasMore()).isTrue();
        assertThat(requested.get().audience()).isEqualTo(BackstageFeedAudience.VENUE);
        assertThat(requested.get().withProviderDeadline(42).audience()).isEqualTo(BackstageFeedAudience.VENUE);
        assertThat(requested.get().viewerProfileId()).isEqualTo(venue);
        ...
        assertThat(codec.decodeForReplay(page.nextCursor(), viewer, TYPES, NOW, BackstageFeedAudience.VENUE, venue).audience())
                .isEqualTo(BackstageFeedAudience.VENUE);
    ...
    @Test void wrongAudienceCursorCannotReachTheReplayJournalEvenAfterAnAccountRoleChange() {
        ...
        assertThatThrownBy(() -> service().getForVenue(viewer, 1, musician, List.of("TRACK")))
    ...
    @Test void musicianEndpointCannotReplayVenueCursorForTheSameViewer() {
        when(guard.requireMusicianProfile(viewer)).thenReturn(UUID.randomUUID());
        assertThatThrownBy(() -> service().get(viewer, 1, token(BackstageFeedAudience.VENUE), List.of("TRACK")))
                .isInstanceOf(SoundConnectException.class);
        verifyNoInteractions(deliveries, personalization, feedback);
    }

    @Test void venueReplayReusesTheValidatedSharedJournalWithoutRedrawingOrReadingPreferences() {
        String token = token(BackstageFeedAudience.VENUE);
        var state = codec.decodeForReplay(token, viewer, TYPES, NOW, BackstageFeedAudience.VENUE, venue);
        var replay = new MusicianFeedPageResponse(1, "venue-v1.0.0", state.feedSessionId(), NOW,
        ...
        assertThat(service().getForVenue(viewer, 1, token, List.of("TRACK"))).isSameAs(replay);
        verify(guard).requireVenueProfile(viewer);
        verifyNoInteractions(personalization, feedback);
    }

    @Test void rejectedVenueCannotReadCandidatesHistoryOrDelivery() {
        when(guard.requireVenueProfile(viewer)).thenThrow(new SoundConnectException(ErrorType.FORBIDDEN_ACCESS));
        assertThatThrownBy(() -> service().getForVenue(viewer, 20, null, List.of("TRACK"))).isInstanceOf(SoundConnectException.class);
        verifyNoInteractions(personalization, deliveries, feedback);
    }

    @Test void changedAnchorVenueCannotReplayPreviouslyCommittedPages() {
        String cursor = token(BackstageFeedAudience.VENUE);
        when(guard.requireVenueProfile(viewer)).thenReturn(UUID.randomUUID());
        assertThatThrownBy(() -> service().getForVenue(viewer, 1, cursor, List.of("TRACK")))
                .isInstanceOfSatisfying(SoundConnectException.class,
                        error -> assertThat(error.getErrorType()).isEqualTo(ErrorType.MUSICIAN_FEED_CURSOR_INVALID));
        verifyNoInteractions(deliveries, personalization, feedback);
    }
    ...
    @Test void completionOnlyCapabilityIsRejectedWithoutLoadingData() {
        assertThatThrownBy(() -> service().getForVenue(viewer, 20, null, List.of("PROFILE_COMPLETION")))
                .isInstanceOf(SoundConnectException.class);
        verifyNoInteractions(guard, deliveries, personalization, feedback);
    }

    @Test void freshContinuationKeepsVenueContextAndWritesTheSharedReplayJournal() {
        ...
        var first = engine.getForVenue(viewer, 1, null, List.of("TRACK"));
        var firstState = codec.decodeForReplay(first.nextCursor(), viewer, TYPES, NOW, BackstageFeedAudience.VENUE, venue);
        ...
        when(deliveries.recordPageAndReplay(eq(viewer), eq(first.feedSessionId()), eq(NOW), eq(1),
                eq("venue-v1.0.0"), eq(1L), anyString(), eq(1), eq(TYPES), anyList(), anyList(), any(), anyBoolean(), eq(NOW)))
                .thenAnswer(call -> new MusicianFeedPageResponse(1, "venue-v1.0.0", first.feedSessionId(), NOW,
                        call.getArgument(9), call.getArgument(11), call.getArgument(12)));
        var next = engine.getForVenue(viewer, 1, first.nextCursor(), List.of("TRACK"));
        assertThat(next.items()).hasSize(1);
        assertThat(next.items().getFirst().id()).isNotEqualTo(first.items().getFirst().id());
        assertThat(next.algorithmVersion()).isEqualTo("venue-v1.0.0");
        assertThat(captured.get().audience()).isEqualTo(BackstageFeedAudience.VENUE);
        assertThat(captured.get().viewerProfileId()).isEqualTo(venue);
        ...
        assertThat(codec.decodeForReplay(next.nextCursor(), viewer, TYPES, NOW, BackstageFeedAudience.VENUE, venue)
    ...
    private String token(BackstageFeedAudience audience) {
        return codec.encode(new MusicianFeedCursorState(viewer, UUID.randomUUID(), NOW,
                ...
                audience == BackstageFeedAudience.VENUE ? venue : null), TYPES);
    }
