...
@WebMvcTest(controllers = OrganizerProfileUserController.class)
@AutoConfigureMockMvc(addFilters = false)
@ActiveProfiles("test")
class OrganizerProfileUserControllerTest {
	...
	void getMyProfile_ok() throws Exception {
		...
		mockMvc.perform(get("/api/v1/user/organizer-profiles/me"))
		       .andExpect(status().isOk())
		       .andExpect(jsonPath("$.success", is(true)))
		       .andExpect(jsonPath("$.code", is(200)))
		       .andExpect(jsonPath("$.data.name").value("OrgMine"));
	}
	...
	void updateMyProfile_ok() throws Exception {
		...
		mockMvc.perform(
				       put("/api/v1/user/organizer-profiles/update")
		       ...
		       .andExpect(jsonPath("$.success", is(true)))
		       .andExpect(jsonPath("$.code", is(200)))
		       .andExpect(jsonPath("$.data.name").value("UpdName"));
	}
}