 ...
 * - Gerçek Postgres (Testcontainers)
 * - Her testte cleanup, sabit zaman
 * - Sıralama, filtre, ownership, batch update gibi tüm işlevler
 */
@Testcontainers
@DataJpaTest
...
class NotificationRepositoryIT {
	...
	// Her test öncesi tablo temizliği, test izolasyonu için şart!
	@BeforeEach
	void cleanup() {
		repo.deleteAll();
		entityManager.clear();
	...
	// Sadece testlerde kolay Notification üretici.
	private Notification make(UUID user, NotificationType type, boolean read,
		...
		Notification n = Notification.builder()
		                             ...
		                             .read(read)
		                             .build();
		// createdAt'i zorunlu set
		org.springframework.test.util.ReflectionTestUtils.setField(n, "createdAt", createdAt);
		return n;
	}
	...
	void saveAndFind_basicCrud_jsonbAndIndexes() {
		...
		Notification saved = repo.save(make(
	...
	void findByRecipient_sortedDesc() {
		// DETERMINISTIC zaman tanımla, CI/CD'de her zaman aynı!
		LocalDateTime now = LocalDateTime.of(2025, 9, 4, 12, 0, 0);
		Notification n1 = repo.save(make(userA, NotificationType.SOCIAL_NEW_FOLLOWER, false, "t1", "m1", Map.of(), now.minusMinutes(3)));
		Notification n2 = repo.save(make(userA, NotificationType.MEDIA_TRANSCODE_READY, true, "t2", "m2", Map.of(), now.minusMinutes(2)));
		Notification n3 = repo.save(make(userA, NotificationType.AUTH_EMAIL_VERIFIED, false, "t3", "m3", Map.of(), now.minusMinutes(1)));
	...
	void countUnread_onlyFalseOnes() {
		repo.save(make(userA, NotificationType.MEDIA_UPLOAD_RECEIVED, false, "u1", "", Map.of(), LocalDateTime.of(2025, 9, 4, 12, 1)));
		repo.save(make(userA, NotificationType.AUTH_EMAIL_VERIFIED, false, "u2", "", Map.of(), LocalDateTime.of(2025, 9, 4, 12, 2)));
	...
	void findTop10ByRecipient_descOrder_limitApplies() {
		...
		for (int i = 0; i < 12; i++) {
			repo.save(make(
					userB,
					NotificationType.MEDIA_UPLOAD_RECEIVED,
					false,
					"b" + i, "m" + i,
					Map.of("i", i),
	...
	void findByTypes_filtering() {
		repo.save(make(userA, NotificationType.MEDIA_TRANSCODE_FAILED, false, "f1", "", Map.of(), LocalDateTime.of(2025, 9, 4, 14, 0)));
	...
	void markAsRead_updatesSingleOwned() {
		Notification unread = repo.save(make(userA, NotificationType.SOCIAL_NEW_FOLLOWER, false, "x", "", Map.of(), LocalDateTime.of(2025, 9, 4, 15, 0)));
	...
	void markAllAsRead_updatesOnlyUsersUnreads() {
		repo.save(make(userB, NotificationType.MEDIA_TRANSCODE_READY, false, "ub1", "", Map.of(), LocalDateTime.of(2025, 9, 4, 15, 1)));
		repo.save(make(userB, NotificationType.MEDIA_TRANSCODE_READY, false, "ub2", "", Map.of(), LocalDateTime.of(2025, 9, 4, 15, 2)));
	...
	void findByIdAndRecipient_securityOwnership() {
		Notification n = repo.save(make(userA, NotificationType.AUTH_RESET_PASSWORD, false, "sec", "", Map.of(), LocalDateTime.of(2025, 9, 4, 16, 0)));
