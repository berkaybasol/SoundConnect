...
| Collab domain | Drain Collab v1 writers, then run `scripts/db/2026-08-11-collab-domain.sql` with `psql -v ON_ERROR_STOP=1` before starting the v2 binary. The script preserves the old listing table as `tbl_collab_legacy_110826`, keeps its dependent slot/target-role rows attached, and creates the actor, listing, application, job, review, saved-listing, and report schema. Reconcile legacy/new row counts, verify every check/unique/partial index, and retain the legacy tables through the observation window; the script intentionally does not guess how v1 slot data should map to the new one-listing/one-need model. |
...
- Collab scale/abuse gate: API page sizes, page depth, text lengths, fee bounds, idempotency keys, one-user-per-listing applications, and the seven-day Extra window are enforced in code and PostgreSQL. Production still requires per-account/IP rate limits for create/apply/report/review actions, realistic `EXPLAIN (ANALYZE, BUFFERS)` and concurrency tests, and a reviewed cap or batched outbox path for the notification fan-out produced when a high-cardinality listing is matched, closed, or expires.
