...
- Studio customer reservation create/list/room-calendar/cancel routes also reject listeners. The service rechecks the canonical account roles and personal profile; creation performs that database check under the existing account lock before reading room, price or reservation data. Existing reservations are retained.
- Avatar navigation may return `profiles: []` with `accessRestriction: "STUDIO_MAINSTAGE_RESTRICTED"` from the public user-profile resolver when studio filtering leaves no accessible target. This allows a neutral explanation screen without exposing studio profile IDs, names or media. Pending-choice and ghost privacy take precedence; mixed responses with accessible targets do not include this marker. Resolver responses use `private, no-store`.
- Listener inbox reads, type filters, recent lists, direct notification access, unread counts and user mutations exclude the explicit business notification types before pagination/counting. Existing studio, Collab, band membership, artist-venue and performer/venue approval notifications remain stored but hidden. Delivery admission and final WebSocket/email dispatch recheck the recipient's current database role through the existing account delivery fence. Social, DM, table, account, media and eligible Overthinking notifications retain their existing behavior.
