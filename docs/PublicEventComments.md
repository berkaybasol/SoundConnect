...
Both return the existing `BaseResponse<Page<...>>` envelope, using `CommentResponseDto` and `CommentReplyResponseDto` respectively. `page` defaults to 0 and accepts 0–1000. `size` defaults to 20 and accepts 1–50. Invalid UUIDs, malformed parameters and values outside these bounds return HTTP 400. Root comments use `createdAt DESC, id DESC` ordering and replies use `createdAt ASC, id ASC`; UUID ties make every page deterministic.
...
The subsequent [all-target comment audit](CommentAudit.md) closes generic target-visibility bypasses, batches authoritative profile avatars and serializes reply creation with parent deletion. Both public and generic page readers now use the same scalar author projection and privacy fences. The historical verification below records the original adapter introduction, not the later audit totals.
